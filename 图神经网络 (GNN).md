...
例如，这种假设在CNN上就是空间变换的不变性，在RNN中则假设的是时序的连续性。

那么，在GNN中，这种假设是置换不变性，或者说点序无关性，前文提到的 Permutation Invariances 。

## 对比聚合操作

无论是求和、取平均值或者求最大值，都无法区分两个网络的输入，或许我们需要更加好的设计，来做到从聚合操作上区分网络的输入（傅里叶变换？）。
