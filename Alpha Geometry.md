...
定理证明对于基于学习的方法来说非常困难，因为在大多数学领域中，能够将人类的证明翻译成机器可验证语言的训练数据及其稀少。不仅如此，几何领域相比于其他奥赛领域更为突出的原因是它几乎没有已经用通用数学化语言（比如Lean）形式化好的证明例子(examples)，这也是由于几何问题中，非常独特的形式化难度。
...
>
>测试基准包括了官方自2000年至今的IMO问题，这些问题是可以被文中所使用的几何环境所表示的(形式化)。通过IMO竞赛成绩从0-7分重新调整到0-1分，人类在竞赛中的表现可以被评估，为的是匹配机器的失败或成功的二进制输出。
>
...
在几何问题中，符号推理引擎是一个推理的数据库(Deductive Database)，它的能力是从前提中，通过几何的规则有效地推理出新的命题。DD遵循的推理规则，是以有限霍恩子句的形式。也就是 $Q(x) \leftarrow P_1 (x) , \dots, P_k (x)$ ，其中，$x$ 是点对象，而 $P_1 (x) , \dots, P_k (x)$ 以及 $Q$ 是谓词，例如”相等“以及”共线“。推理规则的完整列表在ref. 10中可以找到。
...
到目前为止，生成的证明仅仅包括了那些已经可以被高效的符号推理引擎DD+AR达到的推理步骤。然而，为了解决奥赛级别的问题，最关键的缺失部分是生成新的证明项(terms)。
...
作者提出的方法将这种“差异”从P(前提)移动(move 推断)到证明，因而一个用于生成证明的生成模型可以学着去构造它们。
